...
- You will see practical, end-to-end code that "just works" for a concrete use case.
- You are encouraged to tweak prompts, change models, and experiment with the retrieval pipeline.

## What this project does

We use a synthetic retail policy document (`policies.txt`) and build a small RAG system that can answer questions about the store policies.

Core components:
- **Embeddings**: `sentence-transformers` with the `all-MiniLM-L6-v2` model to embed policy paragraphs.
- **Vector store**: `ChromaDB` to store and retrieve paragraph-level embeddings.
...
- `app.py`  
  ...
  - Ensures a local Chroma collection is populated with embedded policy paragraphs.
  - Embeds your questions, retrieves relevant paragraphs, and asks Gemini to answer using only that context.

- `test_chromadb.ipynb`  
  Jupyter notebook to explore the pieces step-by-step:
  - Creating a Chroma collection.
  - Loading and chunking `policies.txt` into paragraphs.
  - Embedding with `sentence-transformers`.
  - Running similarity search queries.

- `policies.txt`  
  Synthetic, detailed retail store policy document used as the knowledge base. Students can read and modify it, or replace it with their own domain text.
...
- Ask questions like: *"What is the return policy on unopened items?"*
- Receive answers generated by Gemini, grounded in the policy text.

Behind the scenes, for each question, the app:
1. Embeds the question.
2. Retrieves the top-k most relevant policy paragraphs from Chroma.
3. Builds a prompt with those paragraphs as context.
...
- **Vibe coding**  
  ...
  - Replace `policies.txt` with another domain (e.g., course handbook, FAQ, docs).
  - Tweak the Streamlit UI to show more or less information.

## Suggested exercises

1. **Change the knowledge base**  
   Replace `policies.txt` with a different document (e.g., your own project policies or a short handbook) and see how the chatbot behaves.
