...
This project shows how to build a small RAG system over **local text documents**. By default, the repo ships with an example retail policy document (`policies.txt`), but you can replace it with any domain text you like (course handbook, FAQ, docs, etc.).
...
- `policies.txt`  
  Example synthetic retail store policy document used as the default knowledge base. Students can read and modify it, or replace it with their own domain text.
...
- Ask questions about the content of your documents (for example: *"What is the return policy on unopened items?"* if you keep the default policies file).
...
1. **Change the knowledge base**  
   Replace `policies.txt` with different documents (e.g., your own project policies, a course handbook, FAQs, or technical docs) and see how the chatbot behaves.
